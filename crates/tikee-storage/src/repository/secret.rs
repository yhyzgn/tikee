...
impl SecretRepository {
    ...
    pub async fn list(&self, namespace: Option<&str>, app: Option<&str>) -> Result<Vec<SecretSummary>, sea_orm::DbErr> {
        let mut query = secret::Entity::find().filter(secret::Column::Status.eq("active"));
        if let Some(namespace) = namespace { query = query.filter(secret::Column::Namespace.eq(namespace)); }
        if let Some(app) = app { query = query.filter(secret::Column::App.eq(app)); }
    ...
    pub async fn create(&self, input: CreateSecret) -> Result<SecretSummary, sea_orm::DbErr> {
        ...
        secret::ActiveModel {
            id: Set(new_id("sec")), namespace: Set(input.namespace), app: Set(input.app), name: Set(input.name),
            value_ref: Set(input.value_ref), status: Set("active".to_owned()), created_by: Set(input.created_by),
            created_at: Set(now.clone()), updated_at: Set(now),
        }.insert(&self.db).await.map(SecretSummary::from)
    }

    pub async fn delete(&self, id: &str) -> Result<bool, sea_orm::DbErr> {
        let Some(existing) = secret::Entity::find_by_id(id.to_owned()).one(&self.db).await? else { return Ok(false); };
        let mut active: secret::ActiveModel = existing.into();
...
impl From<secret::Model> for SecretSummary {
    fn from(value: secret::Model) -> Self {
        Self { id: value.id, namespace: value.namespace, app: value.app, name: value.name, value_ref: value.value_ref, status: value.status, created_by: value.created_by, created_at: value.created_at, updated_at: value.updated_at }
    }
}

//! OIDC callback completion into local opaque tikee sessions.
//!
...
use tikee_storage::{CreateAuditLog, OidcAuthStateRepository, OidcIdentityRepository};
use tracing::warn;
...
async fn consume_oidc_state(
    state: &AppState,
    query: &OidcCallbackQuery,
) -> Result<tikee_storage::OidcAuthStateSummary, ApiError> {
...
async fn mapped_identity(
    ...
    issuer: &str,
    subject: &str,
) -> Result<tikee_storage::OidcIdentitySummary, ApiError> {
...
async fn local_user_for_mapping(
    state: &AppState,
    mapping: &tikee_storage::OidcIdentitySummary,
    subject: &str,
    ...
    state
        ...
        .ok_or_else(|| {
            ApiError::bad_request(format!(
                "OIDC external identity subject {subject} maps to missing local user {username}; no tikee session was created",
...
async fn issue_local_session(
    state: &AppState,
    user: tikee_storage::entities::user::Model,
    mapping: &tikee_storage::OidcIdentitySummary,
) -> Result<AuthSession, ApiError> {
    state
        .sessions
...
fn oidc_scope_bindings(mapping: &tikee_storage::OidcIdentitySummary) -> Vec<AccessScopeBinding> {

...
## Context
Raft config, metadata persistence, fencing-token shape, and a reserved HTTP AppendEntries transport endpoint now exist. The coordinator is still deliberately storage-backed/no-op in `scheduler-server::cluster`; raft mode remains `role=unknown`, `leader_fencing_token=null`, and `can_schedule=false` until a real consensus runtime produces leadership.
...
4. Do not create `scheduler-cluster` crate unless there is a clear stable boundary; document the decision.

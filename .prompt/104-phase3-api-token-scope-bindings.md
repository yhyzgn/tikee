...
## Verification
- RED observed first: `rtk cargo test -p tikee-server api_token_scope_bindings --all-features` failed because `scope_bindings` were ignored.
- Targeted green: `rtk cargo test -p tikee-server api_token_scope_bindings --all-features` passed.
- Broader auth green: `rtk cargo test -p tikee-server api_token --all-features` passed.

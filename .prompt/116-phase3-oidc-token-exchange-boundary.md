...
- Require `access_token` in the token response before any future verification step can proceed.
- Fail closed after exchange until external identity mapping and user mapping are implemented.
- Cover the exchange path with a local mock IdP token endpoint test.

## Out of scope
- UserInfo discovery/cache and signature validation.

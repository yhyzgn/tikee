...
`scripts/management-trigger-e2e-smoke.sh` 是 SDK Management API 创建并触发作业链路的贡献者 smoke。它不是 mock 测试：脚本会启动真实本地 `tikeo` server，创建真实 app-scoped 机器凭证，启动 Node.js demo worker 并通过出站 Worker Tunnel 注册，然后用 Node.js SDK `ManagementClient` 创建作业、执行 `apiTrigger`，最后验证实例结果和持久化日志。

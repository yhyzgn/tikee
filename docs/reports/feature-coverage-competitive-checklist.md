...
| 审计日志 | 🟡 部分覆盖 | `audit_logs` schema；`routes/audit_logs.rs`；多处 CRUD/trigger/login/script gate 写审计；`AuditLogsPage` | 审计日志表、查询/导出、多个关键操作审计已存在 | “所有 CRUD、触发、取消、审批、密钥使用”是否 100% 覆盖未验证；secret 使用审计不明确 | P1 |

...
| `webui/` | 嵌入式 SPA 静态资源 | 由 server 单二进制托管 |
...
Web UI 是平台的默认管理入口，必须覆盖日常开发、运维、排障、审计和治理场景。前端以嵌入式 SPA 随 server 单二进制发布，同时允许独立静态托管以适配企业网关。
...
- 推荐 React + TypeScript + Vite 或 Vue + TypeScript + Vite；最终产物由 `include_dir` 或等价机制嵌入 server。
...
| 前端 | React/Vue + TypeScript + Vite 嵌入式 SPA | — | Web 管理控制台 |
| 嵌入静态资源 | include_dir | 0.7+ | 编译时嵌入前端 |
| OpenAPI | utoipa / aide / schemars | 最新稳定 | OpenAPI 3.1、JSON Schema、Swagger UI |
...
| 前端开发资源 | UI 体验 | 前端独立仓库，社区友好，支持 React/Vue |

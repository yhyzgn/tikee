...
| B-AUTH-001 | 初始化管理员注册/登录 | server auth | smoke 先查 `GET /auth/bootstrap`，未初始化时 `POST /auth/bootstrap/register`，已初始化时 `POST /auth/login` | 返回 `data.token`，且注册入口关闭 | smoke JSON / log | ✅ 通过 | 用户由 smoke 环境变量指定，默认自动生成一次性 smoke 用户 |
...
export SMOKE_USERNAME="${SMOKE_USERNAME:-smoke-$(date +%s)}"
export SMOKE_EMAIL="${SMOKE_EMAIL:-${SMOKE_USERNAME}@example.invalid}"
export SMOKE_PASSWORD="${SMOKE_PASSWORD:-$(openssl rand -base64 24 | tr -d '\n')}"
export AUTH_TOKEN="$(jq -n \
  ...
  | curl -fsS -X POST "$API_URL/api/v1/auth/bootstrap/register" \
